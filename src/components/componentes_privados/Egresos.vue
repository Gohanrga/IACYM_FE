<template>
    <v-layout align-center justify-center wrap row >
        <v-flex xs11 sm11 md11 lg11>
            <span class="headline font-weight-bold primary--text">Egresos</span>
            ...
            >
                <template v-slot:item="prop">
                    ...
                    <tr v-if="prop.item.extendido" style="background: #CFD8DC">
                        <td colspan="11">Pagado a:{{prop.item.pagado}}, Descripción: {{prop.item.descripcion}}</td>
                    </tr>
                    <tr v-if="prop.item.extendido" style="background: #CFD8DC">
        ...
        abreModalEdit(id){
            ...
            for(let i = 0; i < this.listaIngresos.length; i++ ){
                if(this.listaIngresos[i].idIngresoEgreso === id){
                    ...
                    this.varPagado = this.listaIngresos[i].pagado;
